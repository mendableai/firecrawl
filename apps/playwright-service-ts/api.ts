...
const ensureBrowserIsRunning = async () => {
  if (!browser || !context) {
    console.log('🔄 Browser seems closed. Restarting...');
...
app.post('/scrape', async (req: Request, res: Response) => {
  try {
    ...
    return res.status(500).json({ error: 'Browser crashed, restarting, please try again...' });
  }
});
