...
async def test_async_start_crawl_with_options():
    client = AsyncFirecrawl(api_key=os.getenv("API_KEY"), api_url=os.getenv("API_URL"))
    start = await client.v2.start_crawl("https://docs.firecrawl.dev", limit=5, max_discovery_depth=2)
...
async def test_async_start_crawl_with_prompt():
    client = AsyncFirecrawl(api_key=os.getenv("API_KEY"), api_url=os.getenv("API_URL"))
    start = await client.v2.start_crawl("https://firecrawl.dev", prompt="Extract all blog posts", limit=3)
    assert start.id is not None and start.url is not None
...
async def test_async_get_crawl_status_shape():
    client = AsyncFirecrawl(api_key=os.getenv("API_KEY"), api_url=os.getenv("API_URL"))
    start = await client.v2.start_crawl("https://docs.firecrawl.dev", limit=3)
    status = await client.v2.get_crawl_status(start.id)
...
async def test_async_crawl_with_wait():
    client = AsyncFirecrawl(api_key=os.getenv("API_KEY"), api_url=os.getenv("API_URL"))
    job = await client.v2.crawl(url="https://docs.firecrawl.dev", limit=3, max_discovery_depth=2, poll_interval=1, timeout=120)
...
async def test_async_crawl_with_prompt_and_wait():
    client = AsyncFirecrawl(api_key=os.getenv("API_KEY"), api_url=os.getenv("API_URL"))
    job = await client.v2.crawl(url="https://docs.firecrawl.dev", prompt="Extract all blog posts", limit=3, poll_interval=1, timeout=120)
    assert job.status in ("completed", "failed")
    assert job.completed >= 0 and job.total >= 0 and isinstance(job.data, list)
...
async def test_async_crawl_with_scrape_options():
    ...
    start = await client.v2.start_crawl("https://docs.firecrawl.dev", limit=2, scrape_options=scrape_opts)
...
async def test_async_cancel_crawl():
    client = AsyncFirecrawl(api_key=os.getenv("API_KEY"), api_url=os.getenv("API_URL"))
    start = await client.v2.start_crawl("https://docs.firecrawl.dev", limit=3)
    cancelled = await client.v2.cancel_crawl(start.id)
...
async def test_async_get_crawl_errors_and_invalid_job():
    client = AsyncFirecrawl(api_key=os.getenv("API_KEY"), api_url=os.getenv("API_URL"))
    start = await client.v2.start_crawl("https://docs.firecrawl.dev", limit=2)
    errs = await client.v2.get_crawl_errors(start.id)
    assert hasattr(errs, "errors") and hasattr(errs, "robots_blocked")
    with pytest.raises(Exception):
        await client.v2.get_crawl_errors("invalid-job-id-12345")


@pytest.mark.asyncio
async def test_async_active_crawls():
    client = AsyncFirecrawl(api_key=os.getenv("API_KEY"), api_url=os.getenv("API_URL"))
    resp = await client.v2.active_crawls()
...
async def test_async_active_crawls_with_running_crawl():
    ...
    active = await client.v2.active_crawls()
    ids = [c.id for c in active.crawls]
    assert start.id in ids
    # cleanup
    await client.v2.cancel_crawl(start.id)


@pytest.mark.asyncio
async def test_async_crawl_params_preview():
    client = AsyncFirecrawl(api_key=os.getenv("API_KEY"), api_url=os.getenv("API_URL"))
    params = await client.v2.crawl_params_preview(
        url="https://docs.firecrawl.dev",
