...
import { getTopDomains, getDomainFrequency, getDomainFrequencyStats, getDomainFrequencyQueueLength } from '../services';
import { logger } from '../lib/logger';

const router = express.Router();

// Get top domains by frequency
router.get(`/admin/${process.env.BULL_AUTH_KEY}/top`, async (req: Request, res: Response) => {
  try {
...
router.get(`/admin/${process.env.BULL_AUTH_KEY}/domain/:domain`, async (req: Request, res: Response) => {
  try {
    ...
    logger.error('Failed to get domain frequency', { error, domain: req.params.domain });
    res.status(500).json({
      success: false,
...
router.get(`/admin/${process.env.BULL_AUTH_KEY}/stats`, async (req: Request, res: Response) => {
  try {
