...
export const parseTablesToMarkdown = async (html: string): Promise<string> => {
  ...
  if (tables.length) {
    tables.each((_, tableElement) => {
      ...
      let markdownTable: string = convertTableElementToMarkdown(
        cheerio.load(tableElement)
      );
      const isTableEmpty: boolean =
        markdownTable.replace(/[|\- \n]/g, "").length === 0;
      if (isTableEmpty) {
        markdownTable = "";
      }
  ...
  replacements.forEach(({ start, end, markdownTable }) => {
    modifiedHtml =
      modifiedHtml.slice(0, start) +
      `<div>${markdownTable}</div>` +
...
export const convertTableElementToMarkdown = (
  tableSoup: CheerioAPI
): string => {
  let rows: string[] = [];
...
export function convertTableRowElementToMarkdown(
  rowSoup: CheerioAPI,
  rowNumber: number
): string {
