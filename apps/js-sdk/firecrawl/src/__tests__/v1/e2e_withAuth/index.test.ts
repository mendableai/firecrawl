...
describe("FirecrawlApp E2E Tests", () => {
  ...
  test.concurrent(
    ...
    async () => {
      ...
      if (response.metadata !== undefined) {
        ...
        expect(response.metadata.description).toBe(
        ...
        expect(response.metadata.keywords).toBe(
        ...
        expect(response.metadata.ogDescription).toBe(
          "Welcome to Roast My Website, the ultimate tool for putting your website through the wringer! This repository harnesses the power of Firecrawl to scrape and capture screenshots of websites, and then unleashes the latest LLM vision models to mercilessly roast them. 🌶️",
        );
        expect(response.metadata.ogUrl).toBe("https://www.roastmywebsite.ai");
        expect(response.metadata.ogImage).toBe(
  ...
  test.concurrent(
    ...
    async () => {
      const app = new FirecrawlApp({ apiKey: TEST_API_KEY, apiUrl: API_URL });
      const response = await app.crawlUrl("https://roastmywebsite.ai", {}, 30);
      expect(response).not.toBeNull();
  ...
  test.concurrent("should handle idempotency key for crawl", async () => {
    ...
    await expect(
      app.crawlUrl("https://roastmywebsite.ai", {}, 2, uniqueIdempotencyKey),
  ...
  test.concurrent(
    ...
    async () => {
      ...
      while (
        ...
        statusResponse = await app.checkCrawlStatus(response.id);
        expect(statusResponse.success).toBe(true);
        checks++;
      }
  ...
  test.concurrent("should throw error for invalid API key on map", async () => {
    ...
    await expect(
      invalidApp.mapUrl("https://roastmywebsite.ai"),
    ).rejects.toThrow("Request failed with status code 401");
  });
  ...
  test.concurrent(
    ...
    async () => {
      ...
      const response = await app.mapUrl("https://roastmywebsite.ai");
      expect(response).not.toBeNull();
      expect(response.links?.length).toBeGreaterThan(0);
    },
    30000,
  ...
  test.concurrent(
    ...
    async () => {
      ...
      const filteredLinks = response.links?.filter((link: string) =>
        link.includes("roastmywebsite.ai"),
      );
